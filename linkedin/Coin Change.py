...
# 不能重复用
def combinationSum5(nums, s) -> bool:
    dp = [([1] + [0] * s) for _ in range(len(nums) + 1)]
    for i in range(1, len(nums) + 1):
        dp[i][0] = 1
        for j in range(1, s + 1):
            dp[i][j] = dp[i - 1][j] + (dp[i - 1][j - nums[i - 1]] if j >= nums[i - 1] else False)
    return dp[-1][-1]

def combinationSum6(nums, s) -> bool:
    dp = [1] + [0] * s
    for num in nums:
